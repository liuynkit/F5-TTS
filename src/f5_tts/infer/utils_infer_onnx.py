...
def infer_batch_process(
    ...
    for i, gen_text in enumerate(progress.tqdm(gen_text_batches)):
        ...
        with torch.inference_mode():
            text, duration, cond = model_obj.prepare_for_onnx(
                cond=audio,
            ...
            # print("nfe_step size: ", nfe_step.size())
            print("cfg_strength: ", cfg_strength)
            # print("cfg_strength size: ", cfg_strength.size())
            print("sway_sampling_coef: ", sway_sampling_coef)
            # print("sway_sampling_coef size: ", sway_sampling_coef.size())
            print("#" * 50)

            ref_audio_len = torch.tensor([ref_audio_len]).cuda()

            generated_wave, generated_mel_spec = model_obj.inference_for_onnx(
                cond=cond,
                text=text,
                duration=duration,
                steps=nfe_step,
                ...
                vocoder=vocoder)
            # print('generated: ', generated_wave.size())

            generated_wave, _ = model_obj.vocoding_for_onnx(
                generated_wave, vocoder, ref_audio_len)
