...
class ModelWrapper(torch.nn.Module):
    ...
    def forward(self, cond, text, duration, steps, cfg_strength, sway_sampling_coef):
        # inference
...
def main():
    ...
    model = ModelWrapper(ema_model)
    model.eval()

    # Prepare dummy inputs (adjust shapes and types as necessary)
    dummy_cond = torch.randn(1, 240000).cuda()  # Assuming 10 second of audio at 24kHz
    print(dummy_cond)
    ...
    torch.onnx.export(model,
        (dummy_cond, dummy_text, dummy_duration, dummy_steps, dummy_cfg_strength, dummy_sway_sampling_coef),
        "/project/tts/students/yining_ws/multi_lng/F5-TTS/outputs/model.onnx",
        export_params=True,
        opset_version=11,
        do_constant_folding=True,
        input_names=['cond', 'text', 'duration', 'steps', 'cfg_strength', 'sway_sampling_coef'],
        output_names=['output'],
