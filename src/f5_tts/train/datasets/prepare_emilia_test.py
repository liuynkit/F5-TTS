...
en_filters = ["ا", "い", "て"]
fr_filters = ["ふ", "て", "ブ", "ハ", "ヲ", "ゥ", "い", "ギ", "ゼ", "☺", "ソ", "ォ", "ヴ", "ざ", "グ"]
de_filters = ["て", "い", "え", "は", "る", "ㄜ"]
...
def deal_with_audio_dir(audio_dir):
    ...
    with open(audio_jsonl, "r") as f:
        ...
        for line in tqdm(lines, desc=f"{audio_jsonl.stem}"):
            ...
            if obj["language"] == "fr":
                if (
                    any(f in text for f in fr_filters)
                    or repetition_found(text, length=4)
                ):
                    bad_case_fr += 1
                    continue
            if obj["language"] == "de":
                if (
                    any(f in text for f in de_filters)
                    or repetition_found(text, length=4)
                ):
                    bad_case_de += 1
                    continue
    ...
    return sub_result, durations, vocab_set, bad_case_zh, bad_case_en, bad_case_fr, bad_case_de
...
def main():
    ...
    for futures in tqdm(futures, total=len(futures)):
        sub_result, durations, vocab_set, bad_case_zh, bad_case_en,  bad_case_fr, bad_case_de = futures.result()
        result.extend(sub_result)
        duration_list.extend(durations)
        text_vocab_set.update(vocab_set)
        total_bad_case_zh += bad_case_zh
        total_bad_case_en += bad_case_en
        total_bad_case_fr += bad_case_fr
    ...
    if not os.path.exists(f"{data_dir}/data/{dataset_name}"):
        os.makedirs(f"{data_dir}/data/{dataset_name}")
    print(f"\nSaving to {data_dir}/data/{dataset_name} ...")
    # dataset = Dataset.from_dict({"audio_path": audio_path_list, "text": text_list, "duration": duration_list})  # oom
    # dataset.save_to_disk(f"data/{dataset_name}/raw", max_shard_size="2GB")
    with ArrowWriter(path=f"{data_dir}/data/{dataset_name}/raw.arrow") as writer:
        for line in tqdm(result, desc="Writing to raw.arrow ..."):
            writer.write(line)

    # dup a json separately saving duration in case for DynamicBatchSampler ease
    with open(f"{data_dir}/data/{dataset_name}/duration.json", "w", encoding="utf-8") as f:
    ...
    with open(f"{data_dir}/data/{dataset_name}/vocab.txt", "w") as f:
    ...
    if "FR" in langs:
        print(f"Bad en transcription case: {total_bad_case_fr}\n")
    if "DE" in langs:
        print(f"Bad en transcription case: {total_bad_case_de}\n")
...
if __name__ == "__main__":
    ...
    langs = ["DE"]
    dataset_dir = "/project/tts/students/yining_ws/multi_lng/F5-TTS/emilia"
    # "<SOME_PATH>/Emilia_Dataset/raw"
    ...
    # Create a log file
    log_filename = f"{data_dir}/data/{dataset_name}/log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
